...
public class Profiledetail extends AppCompatActivity {
    ...
    Button btn_addwork;
    List<Personaldetails> shelterinMainBeanList;
    ProfileAdapter shelterAdapter;
    ...
    private void callpersonal(String shelterid) {
        ...
        Call<PersonaldetailsMainbean> call = apiServiceSession.getpersonaldata(shelterid);
        Log.e("apiServiceSession_url: ", "" + call.request().url());
        call.enqueue(new Callback<PersonaldetailsMainbean>() {
            @Override
            public void onResponse(Call<PersonaldetailsMainbean> call, Response<PersonaldetailsMainbean> response) {
                hideProgressDialog(progressDiaLogss);
                try {
                    PersonaldetailsMainbean body = response.body();
                    if (body.getPersonalDetails().size()>0) {
                        shelterinMainBeanList = body.getPersonalDetails();
                        final LinearLayoutManager layoutManager = new LinearLayoutManager(getApplicationContext());
                        layoutManager.setOrientation(LinearLayoutManager.VERTICAL);
                        rv_workinitiationlist.setLayoutManager(layoutManager);
                        shelterAdapter = new ProfileAdapter(shelterinMainBeanList, getApplicationContext(), Profiledetail.this);
            ...
            @Override
            public void onFailure(Call<PersonaldetailsMainbean> call, Throwable t) {
                hideProgressDialog(progressDiaLogss);
    ...
    private void showResponseAlert() {
        android.app.AlertDialog.Builder alertDialogBuilder = new android.app.AlertDialog.Builder(Profiledetail.this);
